...
namespace e
{
	namespace lib
	{
		namespace krnln
		{
			...
			template<class T, std::enable_if_t<std::is_arithmetic_v<T>, int> = 0> inline void Print(std::optional<int32_t> 输出方向, T 欲输出内容)
			{
			...
			template <typename Arg1, typename Arg2, typename ... MoreArgs> void Print(std::optional<int32_t> 输出方向, const Arg1& arg1, const Arg2& arg2, const MoreArgs&... moreArgs)
			{
				int dummy[] = { 
					(Print(输出方向, arg1), 0),
					(Print(输出方向, arg2), 0),
